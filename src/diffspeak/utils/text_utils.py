...
def load_embeddings(
    embedding_path: str, embedding_type: str = "fasttext"
) -> Union[Dict, Any]:
    """
...
def get_vector(
    embedding_type: str, embedding_index: dict, word: str
...
def build_matrix(
    ...
    if embeddings_type in ["word2vec", "glove"]:
        embed_size = (
            ...
            else len(list(embedding_index.values())[0])
        )
    ...
    for word, i in word_dict.items():
        ...
        if embedding_vector is not None:
            embedding_matrix[i] = embedding_vector
            continue
        embedding_vector = get_vector(
            embeddings_type, embedding_index, word.capitalize()
        )
        if embedding_vector is not None:
            embedding_matrix[i] = embedding_vector
            continue
