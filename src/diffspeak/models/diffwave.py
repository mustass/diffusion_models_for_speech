...
class DiffWave(nn.Module):
    def __init__(self, cfg):
        ...
        if self.params.unconditional:  # use unconditional model
            ...
            self.spectrogram_upsampler = SpectrogramUpsampler(self.params.n_mels)

        self.residual_layers = nn.ModuleList(
            [
                ResidualBlock(
                    self.params.n_mels,
                    self.params.residual_channels,
                    2 ** (i % self.params.dilation_cycle_length),
                    uncond=self.params.unconditional,
                )
                for i in range(self.params.residual_layers)
            ]
        )
        self.skip_projection = Conv1d(
            self.params.residual_channels, self.params.residual_channels, 1
        )
        self.output_projection = Conv1d(self.params.residual_channels, 1, 1)
